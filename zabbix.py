...
class zabbix_api: 
	...
	def host_create(self, hostip, hostgroupName, templateName,hostName): 
	    ...
	    try: 
	        ...
	        response = json.loads(result.read()) 
	        result.close() 
	        print "add host : \033[42m%s\031[0m \tid :\033[31m%s\033[0m" % (hostip, response['result']['hostids']) 
...
if __name__ == "__main__":
	...
	parser.add_argument('-C','--add-host',dest='addhost',nargs=4,metavar=('192.168.2.1', 'groupname', 'Template01,Template02', 'hostName'),help='添加主机,多个主机组或模板使用分号')
